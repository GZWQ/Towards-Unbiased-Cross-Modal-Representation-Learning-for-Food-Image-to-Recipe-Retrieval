...
def computeAverageMetrics(imfeats, recipefeats, k, t, forceorder=False):
    ...
    for tt in range(t):
        ...
        if not forceorder and tt == 0:
            print("ids ",sub_ids[:5], sub_ids[-5:])
        ...
        # print(tt)
        # print(glob_metrics)
        # for kk, vv in glob_metrics.items():
        #     print(kk + ':', np.mean(vv))
    return glob_metrics
...
#
#     args = get_eval_args()
#     args.embeddings_file = os.path.join(args.embeddings_file, args.exp_choice, args.model_name, 'feats_test.pkl')
#     eval(args)
