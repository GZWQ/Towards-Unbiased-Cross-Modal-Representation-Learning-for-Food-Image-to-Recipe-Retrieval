...
def dump_pkl(file_path, file):
    print('saving ', file_path)
    with open(file_path, 'wb') as f:
        pickle.dump(file, f, protocol=4)

def pretty_count_parameters(model):
    ignore_keys = ['text_encoder', 'image_encoder', 'merger_recipe', 'projector_recipes']
    ...
    for name, parameter in model.named_parameters():
        ...
        tokens = name.split('.')
        if tokens[0] not in ignore_keys:
