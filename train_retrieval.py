...
def main_worker(args, logger):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.val_interval == 0:
            ...
            loss, recalls = validate(val_loader, model, criterion, args, logger)
            losses.update(loss)
...
def train(train_loader, model, criterion, ranking_loss, optimizer, epoch, args, logger):
    ...
    for i, (images, target, img_idx, title, ingrs, instrs) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            output, img_feat, recipe_feat, proj_feats, _, _ = model(images, title, ingrs, instrs)
...
def get_metrics_mmres(args, img_feats, recipe_feats):
...
def get_metrics(args, img_feats, recipe_feats):
...
def eval_F1(args, results, gt_labels, ids=None, split='val'):
    ...
    if args.evaluate:
        ...
        save_path = os.path.join(args.output, 'id2labels_preds_ir2_{}.pkl'.format(split))
...
def validate(val_loader, model, criterion, args, logger, split='val'):
    ...
    model.eval()
    img_feats_cls, labels, allids = None, None, []
    img_feats_total, recipe_feats_total = None, None
    ...
    with torch.no_grad():
        ...
        for i, (images, target, ids, title, ingrs, instrs) in enumerate(val_loader):
            ...
            # compute output
            with torch.cuda.amp.autocast(enabled=args.amp):
                output, img_feat, recipe_feat, _, _, _ = model(images, title, ingrs, instrs)
            ...
            if img_feats_cls is not None:
                img_feats_cls = np.vstack((img_feats_cls, output.cpu().detach().numpy()))
                ...
                img_feats_cls = output.cpu().detach().numpy()
        ...
        # get_metrics_analysis(args, img_feats_total, recipe_feats_total, allids)
        # exit(0)
        avg_metrics = get_metrics(args, img_feats_total, recipe_feats_total)

    return loss_avg, avg_metrics
