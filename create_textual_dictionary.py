...
def get_args():
    ...
    suf = ''
    if args.cls_weight > 0.0:
        suf += 'Cls'
    if args.retrieval_weight > 0.0:
        suf += 'Retrieval'

    size_aug = 'Size{}_Aug{}'.format(args.img_size, args.aug_type)
    args.output = os.path.join(args.output, args.dataname, args.backbone, size_aug, suf)
    args.pretrained_path = args.output
...
def main():
    ...
    # args.output = os.path.join(args.output, args.dataname, args.backbone, suf)
    ...
    logger = setup_logger(output=args.output, distributed_rank=dist.get_rank(), color=False, name='dictionary_creation')
...
def main_worker(args, logger):
    ...
    validate(train_loader, model, criterion, args, logger, split='train')
    ...
    validate(val_loader, model, criterion, args, logger, split='val')
    ...
    validate(test_loader, model, criterion, args, logger, split='test')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.val_interval == 0:
            ...
            loss, prf, recalls = validate(val_loader, model, criterion, args, logger)
            losses.update(loss)

            pres.update(prf['precision'])
            recs.update(prf['recall'])
            f1s.update(prf['f1'])
            ...
            precision_list.append(prf['precision'])
            recall_list.append(prf['recall'])
...
def eval_F1(args, results, gt_labels, ids=None, split='val'):
    ...
    p = precision_score(gt_labels, y_pred, average='macro')
    r = recall_score(gt_labels, y_pred, average='macro')

    # p_clswise = precision_score(gt_labels, y_pred, average=None)
    # r_clswise = recall_score(gt_labels, y_pred, average=None)

    if args.evaluate:

        id2preds = {}
        if ids is not None:
            for pred, id in zip(y_pred, ids):
                id2preds[id] = pred.nonzero()[0].tolist()

        save_path = os.path.join(args.pretrained_path, 'id2labels_preds_{}.pkl'.format(split))
        utils.dump_pkl(save_path, id2preds)
    return macro_f1, {"precision": p, "recall": r, 'f1': 2 * p * r / (p + r)}

def step4_generate_ingrs_dict(args, id2feats):
    id2pos = utils.load_pkl('/common/home/users/q/qingwang.2020/codes/HT_Debiasing/data/recipe1M/id2pos_train.pkl')

    ingredient_dict = defaultdict(list)

    print("id2pos vs id2feats: ",len(id2pos), len(id2feats))
    for id, pos in id2pos.items():
        feats = id2feats[id]
        for pos_pair in pos:
            for k, v in pos_pair.items():
                if k < len(feats):
                    ingredient_dict[v].append(feats[k])

    for k,v in ingredient_dict.items():
        if len(v)==0:
            ingredient_dict[k] = ingredient_dict[k-1]

    print(len(ingredient_dict))
    utils.dump_pkl(os.path.join(args.pretrained_path, 'ingredient_dict_textual.pkl'), ingredient_dict)

@torch.no_grad()
def validate(val_loader, model, criterion, args, logger, split='val'):
    ...
    with torch.no_grad():
        ...
        for i, (images, target, ids, title, ingrs, instrs) in enumerate(val_loader):
            ...
            # compute output
            with torch.cuda.amp.autocast(enabled=args.amp):
                output, img_feat, recipe_feat, projected_text_features, ingredient_feats,_ = model(images, title, ingrs, instrs)
                loss = criterion(output, target)
                if args.loss_dev > 0:
                    loss *= args.loss_dev

                ingredient_feats = ingredient_feats.view(images.size(0), -1, ingredient_feats.size(-1))
                ingredient_feats_cpu = ingredient_feats.cpu().detach().numpy()
                for idx, one_id in enumerate(ids):
                    id2ing_l1[one_id] = ingredient_feats_cpu[idx]
        ...
        avg_metrics = get_metrics(img_feats_total, recipe_feats_total)
        text_ = "Retrieval. "
        values = []
        ...
        if split == 'train':
            step4_generate_ingrs_dict(args, id2ing_l1)
...
if __name__ == '__main__':
    # python create_textual_dictionary.py --dist-url tcp://127.0.0.1:3707 --dataname semantic_food_500 --backbone resnet50 --evaluate
    main()
